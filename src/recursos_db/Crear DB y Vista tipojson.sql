...
CREATE TABLE `catalogo` (
	`catalogo_id` INT NOT NULL AUTO_INCREMENT UNIQUE,
...
CREATE TABLE `catalogo_genero` (
	`catalogo_genero_id` INT NOT NULL AUTO_INCREMENT UNIQUE,
...
DROP TABLE IF EXISTS `catalogo_actores`;
CREATE TABLE `catalogo_actores` (
	`catalogo_actores_id` INT NOT NULL AUTO_INCREMENT UNIQUE,
	`catalogo_id` INT NOT NULL,
	`actores_id` INT NOT NULL,
	PRIMARY KEY (`catalogo_actores_id`),
	FOREIGN KEY (`catalogo_id`) REFERENCES `catalogo`(`catalogo_id`),
...
INSERT INTO catalogo(catalogo_id, poster, titulo, categoria_id, resumen, temporadas, trailer) 
values 
  ( 
    1,
    "/posters/1.jpg",
...
INSERT INTO catalogo_actores (catalogo_id, actores_id) 
VALUES 
(1,25),
(1,26),
(1,27),
...
-- Creacion de vista como el json--
...
SELECT
    ...
    (
        ...
        FROM generos g
        JOIN catalogo_genero cg ON g.generos_id = cg.genero_id
        WHERE cg.catalogo_id = c.catalogo_id
    ) AS genero,
    c.resumen,
    c.temporadas,
...
JOIN
    catalogo_actores ca ON c.catalogo_id = ca.catalogo_id
JOIN
    actores a ON ca.actores_id = a.actores_id
GROUP BY
    c.catalogo_id, c.poster, c.titulo, cat.categoria_nombre, c.resumen, c.temporadas, c.trailer;
